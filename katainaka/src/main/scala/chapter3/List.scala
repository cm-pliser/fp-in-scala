...
object List {
  ...
  def apply[A](as: A*): List[A] = {
    as.reverse.foldLeft[List[A]](Nil) { (b, a) =>
      Cons(a, b)
    }
  }
  ...
  def drop[A](as: List[A], n: Int): List[A] =
    n match {
      case _ if n < 0 =>
        throw new IllegalArgumentException("Negative number is passed")
      case 0 => as
      case _ =>
        as match {
          case Cons(_, t) => drop(t, n - 1)
          case Nil =>
            throw new IllegalArgumentException("Passed list is smaller than n")
        }
    }
  ...
  def foldRightShortCircuit[A, B](list: List[A],
                                  z: => B)(f: (A, => B) => B): B = list match {
    case Cons(a, as) => f(a, foldRightShortCircuit(as, z)(f))
    case Nil         => z
  }

  def foldRightShortCircuit2[A, B](list: List[A], z: () => B)(
    f: (A, () => B) => B
  ): B = list match {
  ...
  def lengthByFoldLeft(list: List[Int]): Int =
    foldLeft(0, list)((b, _) => b + 1)

  def reverse[A](list: List[A]): List[A] =
    foldLeft[A, List[A]](Nil, list)((b, a) => Cons(a, b))

  def foldRightBasedOnFoldLeft2[A, B](list: List[A], z: B)(f: (A, B) => B): B =
    foldLeft[A, B => B](identity, list)((next, a) => (f(a, _: B)) andThen next)(
      z
    )
  ...
  def foldRightBasedOnFoldLeft3[A, B](list: List[A],
                                      z: B)(f: (A, B) => B): B = {
  ...
  def foldLeftBasedOnFoldRight[A, B](z: B, list: List[A])(f: (B, A) => B): B =
    foldRight[A, B => B](list, identity)((a, acc) => (f(_: B, a)) andThen acc)(
      z
    )
  ...
  def filter[A](as: List[A])(cond: A => Boolean): List[A] =
    as match {
      ...
      case Cons(_, tail)                  => filter(tail)(cond)
      case Nil                            => Nil
    }
  ...
  def tailsWithList[A](list: List[A]): List[List[A]] =
    scanRight[A, List[A]](list, Nil)(Cons(_, _))
