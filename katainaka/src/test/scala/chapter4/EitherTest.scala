...
class EitherTest extends WordSpec with GeneratorDrivenPropertyChecks {
  ...
  implicit def leftArb[E: Arbitrary]: Arbitrary[Left[E]] =
    Arbitrary(Gen.resultOf[E, Left[E]](Left(_)))

  implicit def rightArb[A: Arbitrary]: Arbitrary[Right[A]] =
    Arbitrary(Gen.resultOf[A, Right[A]](Right(_)))

  implicit def eitherArb[E: Arbitrary, A: Arbitrary]: Arbitrary[Either[E, A]] =
    Arbitrary(Gen.oneOf(arbitrary[Left[E]], arbitrary[Right[A]]))
  ...
  "sequence" should {
    ...
    "Leftを含むなら最初のLeft" in
      forAll {
        (preffix: List[Right[Int]],
         suffix: List[Either[String, Int]],
         message: String) =>
          Either.sequence(preffix ++ (Left(message) :: suffix)) shouldBe Left(
            message
          )
      }
  }
  ...
  "traverse" should {
    ...
    "Leftを含むならば最初のLeft" in
      forAll { (preffix: List[Int], N: Int, suffix: List[Int], fail: Int => Left[String], success: Int => Right[Double], any: Int => Either[String, Double]) =>
        val nIsFirstLeft: PartialFunction[Int, Either[String, Double]] = {
          case N                        => fail(N)
          case n if preffix.contains(n) => success(n)
        }

        Either.traverse(preffix ++ (N :: suffix))(
          nIsFirstLeft.orElse(PartialFunction(any))
        ) shouldBe fail(N)
      }
  }
}
